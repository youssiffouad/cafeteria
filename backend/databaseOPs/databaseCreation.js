...
// });
db.serialize(() => {
  // Drop the existing OrderItems table
  db.run(`DROP TABLE IF EXISTS OrderItems`, function(err) {
    if (err) {
      return console.error(err.message);
    }
    console.log('Table OrderItems dropped');
  });

  // Create the OrderItems table with the new column and foreign key constraints
  db.run(`
  ...
  `, function(err) {
    ...
    console.log('Table OrderItems created with sandwich_id column and foreign key constraints');
  });
});
